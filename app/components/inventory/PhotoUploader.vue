 ...
 *  - CREATE mode (no itemId): a single file input. The chosen File is emitted via
 *    v-model and held by the parent ItemForm, which passes it to
 *    saveItem(form, photoFile) — the store then auto-POSTs it as a SECOND request
 *    behind ONE success surface (D-07). Nothing is uploaded from here directly.
...
const props = defineProps<{
  /** CREATE mode: the staged single File (v-model). */
...
// ---------------------------------------------------------------
// CREATE mode — stage a single File + preview
// ---------------------------------------------------------------
...
<template>
  <!-- CREATE MODE: single optional photo, staged for the seamless save -->
  <div v-if="!isEdit" class="space-y-2">
    <div v-if="previewUrl" class="flex items-center gap-3">
      <img :src="previewUrl" alt="" class="size-24 rounded-md object-cover">
      <Button type="button" variant="outline" size="sm" @click="clearCreate">
